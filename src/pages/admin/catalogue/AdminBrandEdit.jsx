...
const styles = {
  ...
  logoPreview: { width: '80px', height: '80px', objectFit: 'contain', borderRadius: '6px', border: '1px solid #e9ecef', marginBottom: '8px', display: 'block' }
};
...
export default function AdminBrandEdit() {
  ...
  const loadBrand = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/brands/' + id, { headers: { Authorization: 'Bearer ' + token() } });
      if (!res.ok) throw new Error('Failed to load brand');
      const data = await res.json();
      const b = data.data || data.brand || data;
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      setSubmitSuccess('Brand saved successfully.');
    } catch (e) { setSubmitError(e.message); }
    finally { setSubmitting(false); }
  };

  if (loading) return <div style={styles.page}><div style={styles.container}>Loading brand...</div></div>;
          ...
          <div style={styles.card}>
            {form.logo_url && (
              <div style={styles.formGroup}>
                <img src={form.logo_url} alt="Brand logo" style={styles.logoPreview} onError={e => { e.target.style.display = 'none'; }} />
              </div>
            )}
            <div style={styles.formGroup}>
            ...
            <div style={styles.formGroup}>
              ...
              <input id="slug" name="slug" value={form.slug} onChange={handleChange} style={styles.input} />
            </div>
            <div style={styles.formGroup}>
