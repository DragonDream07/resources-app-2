...
export default function AdminCategoryEdit() {
  ...
  const loadCategory = useCallback(async () => {
    ...
    try {
      ...
      const c = data.data || data.category || data;
      setForm({ name: c.name || '', slug: c.slug || '', parent_id: c.parent_id != null ? String(c.parent_id) : '' });
  ...
  useEffect(() => { loadCategory(); loadCategories(); }, [id]);
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      setSubmitSuccess('Category saved successfully.');
    } catch (e) { setSubmitError(e.message); }
  ...
  if (loading) return <div style={styles.page}><div style={styles.container}>Loading category...</div></div>;
            ...
            <div style={styles.formGroup}>
              ...
              {catsLoading ? (
                ...
                <select id="parent_id" name="parent_id" value={form.parent_id} onChange={handleChange} style={styles.select}>
                  ...
                  {flattenCategories(categories).map(c => <option key={c.id} value={String(c.id)}>{c.name}</option>)}
                </select>
              )}
            </div>
          </div>
